        ...
        async def wrapper(*args, **kwargs):
            for i in range(retries):
                try:
                    ...
                    logging_operations.log_local('WARN', f'Retrying in {func.__name__}. Attempt {i+1} out of {retries}', file=stderr)
            await logging_operations.log_local_and_remote('ERROR', f'At {func.__name__}: Could not reach server after {retries} attempts')
...
async def initialize_accesspoint(session):
    ...
    try:
        async with session.post('/api/access-points', json=data) as response:
            ...
            await logging_operations.log_local_and_remote('INFO', f'Initialized access point for IP: {common.web_server_address}')
...
async def get_ap_status(session):
    async with session.get('/api/access-points/' + common.access_point_name) as response:
        try:
            ...
            status = data['status']
            await logging_operations.log_local_and_remote('INFO', f'Retrieved access point status: {status}')
            return status
        except json.decoder.JSONDecodeError as e:
            await logging_operations.log_local_and_remote('ERROR', f'Could not parse json in get_ap_status. Error: {e}')
            return None
...
async def get_sensorstation_instructions(session):
    ...
    async with session.get('/api/access-points/' + common.access_point_name + '/sensor-stations') as response:
        try:
            ...
            await logging_operations.log_local_and_remote('ERROR', f'Could not parse json in get_sensorstation_instructions. Error: {e}')
            return paired_stations
        except KeyError as e:
            await logging_operations.log_local_and_remote('ERROR', f'KeyError in get_sensorstation_instructions. Error: {e}')
            return paired_stations
    await logging_operations.log_local_and_remote('INFO', f'Retrieved sensorstation instructions. SS: {paired_stations}')
    return paired_stations
...
async def send_sensorstations_to_backend(session, sensorstations):
    ...
    async with session.post('/api/access-points/' + common.access_point_name + '/sensor-stations', json=ss_avail) as response:
        await logging_operations.log_local_and_remote('INFO', f'Available sensorstations sent to web server. Sensorstations: {ss_avail}')
...
async def send_sensorstation_connection_status(session, sensorstation, status):
    ...
    async with session.put('/api/sensor-stations/' + str(sensorstation), json=ss_status) as response:
        await logging_operations.log_local_and_remote('INFO', f'Updated sensorstation connection status for station: {sensorstation} to {status}',entity_type='SENSOR_STATION', entity_id=str(sensorstation))
...
async def send_warning_to_backend(sensorstation_id, session):
    ...
    async with session.put('/api/sensor-stations/' + str(sensorstation_id), json=data) as response:
        await logging_operations.log_local_and_remote('WARN', f'Updated sensorstation status for station: {sensorstation_id} to WARNING', entity_type='SENSOR_STATION', entity_id=str(sensorstation_id))
...
async def clear_warning_on_backend(sensorstation_id, session, data):
    if int.from_bytes(data, 'little', signed=False) == 0:
        ...
        async with session.put('/api/sensor-stations/' + str(sensorstation_id), json=data) as response:
            await logging_operations.log_local_and_remote('INFO', f'Cleared warning on the web server for sensorstation: {sensorstation_id}. Status set to: OK', entity_type='SENSOR_STATION', entity_id=str(sensorstation_id))
...
async def get_thresholds_update_db(sensorstation_id, session):
    async with session.get('/api/sensor-stations/' + str(sensorstation_id)) as response:
        ...
        await logging_operations.log_local_and_remote('INFO', f'Updated thresholds for sensorstation: {sensorstation_id}. Thresholds: {json_data}', entity_type='SENSOR_STATION', entity_id=str(sensorstation_id))
...
async def send_sensorvalues_to_backend(sensorstation_id, session):
    ...
    if averages_dict:
        ...
        async with session.post('/api/sensor-stations/' + str(sensorstation_id) + '/measurements', json=averages_dict) as response:
            ...
            await logging_operations.log_local_and_remote('INFO', f'Sent sensor values to web server for sensorstation: {sensorstation_id}. Values: {averages_dict}', entity_type='SENSOR_STATION', entity_id=str(sensorstation_id))
    else:
        await logging_operations.log_local_and_remote('ERROR', f'Could not accumulate sensor values for sensorstation: {sensorstation_id}', entity_type='SENSOR_STATION', entity_id=str(sensorstation_id))
...
async def send_logs(session, logging_data):
    async with session.post('/api/access-points/' + common.access_point_name + '/logs', json=logging_data) as response:
        ...
        await logging_operations.log_local('INFO', f'Sent logs to web server.')
