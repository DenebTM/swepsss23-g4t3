...
async def read_sensorvalues(client, sensorstation_id, connection_request):
    while not connection_request.done() and client.is_connected:
        try:
            ...
            await logging_operations.log_local_and_remote('ERROR', f'BleakError while retreiving sensorvalues from sensorstation: {sensorstation_id}. Error: {e}', entity_type='SENSOR_STATION', entity_id=str(sensorstation_id))
